...
def saveImg(path, array, F_normal=False):
    if F_normal:
        Max, Min = np.max(array), np.min(array)
        dst = (array - Min) / (Max - Min)
        cv2.imwrite(path, dst * 255)
    else:
...
def parse_args():
    ...
    parser.add_argument(
        "-dPath", "--depthWritePath", help="path to save depth map", type=str
    )
    parser.add_argument(
        "-nPath", "--normalWritePath", help="path to save normal map", type=str
    )
    parser.add_argument(
...
def main():
    ...
    saveImg(path=args.depthWritePath, array=depth_map, F_normal=True)
    saveImg(path=args.normalWritePath, array=normal_map, F_normal=True)
